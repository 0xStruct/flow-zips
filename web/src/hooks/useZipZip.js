...
import ZIP_ZIP_TX_CDC from "cadence/transactions/zip_zip.cdc"
...
export default function useZipZip(itemID) {
  ...
  const zipZip = async (item, zipData) => {
    if (!item) throw new Error("Missing item")
    if (!zipData) throw new Error("Missing zipData")

    const newTxId = await fcl.mutate({
      cadence: ZIP_ZIP_TX_CDC,
      args: (arg, t) => [
        arg(item.itemID.toString(), t.UInt64),
        arg(zipData.toString(), t.String),
      ],
      limit: 1000,
    })
